...
/// StoreKit 2 — one auto-renewable subscription (`sprout_pro_monthly`, $0.99/month).
...
final class Store: ObservableObject {
    static let productID = "sprout_pro_monthly"
    ...
    private var debugForcePro: Bool {
        #if DEBUG
        return ProcessInfo.processInfo.environment["SPROUT_FORCE_PRO"] == "1"
        #else
        return false
        #endif
    }
    ...
    init() {
        ...
        Task {
            #if DEBUG
            // SPROUT_NO_SK skips the product fetch so headless screenshot runs (no StoreKit config /
            // no App Store account) don't trigger the system "Sign in to Apple Account" prompt.
            if ProcessInfo.processInfo.environment["SPROUT_NO_SK"] != "1" { await loadProduct() }
            #else
            await loadProduct()
            #endif
